...
async function fetchPlayURL(channel, region, environment, cdn) {
    ...
    if (!channelsData.channels[channel]) {
        console.error(`Error: Channel '${channel}' not found in JSON.`);
        return null;
    }
    
    // Get channel config based on environment
    const channelConfig = channelsData.channels[channel];
    if (!channelConfig.env || !channelConfig.env[environment]) {
        console.error(`Error: Environment '${environment}' not found for channel '${channel}'.`);
        return null;
    }
    
    const envConfig = channelConfig.env[environment];
    const url = `${envConfig.path}cdn=${cdn}&channel=${channel}&mcl_region=${region}`;
    
    const postData = {
        pattern: channelConfig.data.pattern
    };
