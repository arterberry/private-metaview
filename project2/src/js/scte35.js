...
// Light wrapper for inspector integration
window.SCTE35 = {
    parseFromPayload: function(bodyText) {
        try {
            // Attempt to extract SCTE data from body as hex
            const hexMatch = bodyText.match(/[0-9a-fA-F]{20,}/g); // crude hex filter
            if (!hexMatch || !hexMatch.length) return [];
            ...
            const parsed = SCTE35Parser.parseFromBytes(byteArray);
            if (parsed && !parsed.error) {
                return [parsed]; // return as array for batch-safe handling
            }
        } catch (err) {
            console.error("SCTE35 parseFromPayload failed:", err);
        }
        return [];
    }
};
